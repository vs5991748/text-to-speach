...
def _call_llm(cfg: dict, word: str, learning_lang: str, translation_lang: str, count: int = 1, instructions: str = "") -> list:
    ...
    if count == 0:
        ...
        prompt = (
            ...
            f'Use it EXACTLY ONCE per sentence, in the one grammatical form that sentence is required to '
            f'demonstrate — never repeat the word in a second, different form within the same sentence. '
            f'Each sentence should be {sentence_style}, and must be grammatically correct and make logical '
            f'sense — never an artificial, redundant, or self-contradictory construction (e.g. do not write '
            f'something equivalent to "let us forget not to forget" just to fit the word in twice). '
            f'A command or wish form ("let us…", "do…!") only makes sense for something the speaker could '
            f'plausibly want or control — if the word describes an involuntary action (e.g. forgetting, '
            f'losing, failing), phrase that required sentence as advice or a warning addressed to someone '
            f'else, or a common idiom, instead of an illogical self-command to intentionally do something '
            f'nobody can do on purpose. '
